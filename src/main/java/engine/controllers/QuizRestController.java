...
public class QuizRestController {
    ...
    public ResponseEntity<Quiz> addQuiz(@Valid @RequestBody Quiz quizToSave, @Autowired Principal principal) {
        return new ResponseEntity<>(quizService.saveQuiz(quizToSave, (principal != null) ? principal.getName() : "Guest"), HttpStatus.OK);
    }
    ...
    public ResponseEntity<Page<Completed>> getCompletedQuizzes(@RequestParam(defaultValue = "0") int page,
                                                               @Autowired Principal principal) {
        return new ResponseEntity<>(quizService.getCompletedQuizzes(((principal != null) ? principal.getName() : "Guest"), page), HttpStatus.OK);
    }
